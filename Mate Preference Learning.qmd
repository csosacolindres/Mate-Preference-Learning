...
mPrefVector <- matrix(startPref, nrow(males), 15)
colnames(mPrefVector) <- c("prefVector_affectionate", 
                            "prefVector_ambition", 
                            "prefVector_artistic", 
...
fPrefVector <- matrix(startPref, nrow(females), 15)
colnames(fPrefVector) <- c("prefVector_affectionate", 
                            "prefVector_ambition", 
                            "prefVector_artistic", 
                            "prefVector_disposition", 
                            "prefVector_family", 
...
#AGENTS
mAgents <- cbind(mTraits, mPrefVector, mPrefMemory)
fAgents <- cbind(fTraits, fPrefVector, fPrefMemory)
...
randomMethod <- function(mPrefVector, fPrefVector){
  
  mRandom <- t(apply(mPrefVector, 1, function(y) runif(y, min = 0, max = 10)))
  colnames(mRandom) <- colnames(mPrefVector)
  mPrefVector <- mRandom
  
  fRandom <- t(apply(fPrefVector, 1, function(x) runif(x, min = 0, max = 10)))
  colnames(fRandom) <- colnames(fPrefVector)
  fPrefVector <- fRandom
  
  return(list(mPrefVector, fPrefVector))
}
...
for(p in 1:prefLearningCycles){
  if(p > 1) {
   randomMethod(mPrefVector, fPrefVector) 
  }
  
  # Calculate MV
  mMVs <- t(apply(mPrefVector, 1, function(x) apply(fTraits, 1, function(y) mvcalc(x, y))))
  fMVs <- t(apply(fPrefVector, 1, function(x) apply(mTraits, 1, function (y) mvcalc(x, y))))
 ...
 if(mRewMemory < mRewVector){
    
    # for every "False" do this: 
    
  
    
    mRewMemory[mMaxReward] <- mRewVector[mMaxReward]

    fRewMemory[fMaxReward] <- fRewVector[fMaxReward]
