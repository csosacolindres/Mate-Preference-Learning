...
males <- males[sample(1:nrow(males), 5),]
females <- females[sample(1:nrow(females), 5),]
...
##### ANALYSIS #####
# Rewrite # Apply function to standardize actual and model prefs, melt into 
...
fit <- stan_glmer(predicted_value ~ observed_value + (1|id), data = d)
fitSummary <- summary(fit)

fitZ <- stan_glmer(predicted_value ~ observed_value + (1|id), data = d_z)
fitSummaryZ <- summary(fitZ)


plotFit <- ggplot(d_z, aes(y=predicted_value, x=observed_value)) + geom_point() + geom_smooth(method = "lm")
